...
class DASHWorker(AbstractWorker):
    ...
    def start(self, info: StreamInfoObject):
        ...
        if verification_backup_path and verification_target_filename:
            last_processed_seq, current_stream_time, process = self._verify_stream_continuity(
                ...
                state_file,
                process,
                verification_backup_path,
                verification_target_filename,
                last_processed_seq,
                current_stream_time,
            )
    ...
    def _verify_stream_continuity(
        self,
        info: StreamInfoObject,
        fragment_dir: str,
        state_file: str,
        process: subprocess.Popen,
        verification_backup_path: str,
        ...
        if new_frag_path is None:
            # Timeout
            logger.warning(
                f"[{info.key}][DASHWorker] Verification Timeout: New Fragment 1 did not appear. Assuming stream is far ahead (No reset)."
            )
            # Restore backup
            target_path = os.path.join(fragment_dir, verification_target_filename)
